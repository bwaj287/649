...
The trainer writes `trained_model_config.json` and `train_results/`. Future predictions automatically use `trained_model_config.json` unless disabled:
...
- `train_model_weights.mjs`: searches model weights with train/validation/test splits and writes a local trained config.
...
运行本地随机权重搜索，并使用训练/验证/测试切分：

```powershell
node .\train_model_weights.mjs
```

训练会生成 `trained_model_config.json` 和 `train_results/`。之后的预测会自动使用 `trained_model_config.json`，除非手动关闭：
...
- `train_model_weights.mjs`：使用训练/验证/测试切分搜索模型权重，并写入本地训练配置。
